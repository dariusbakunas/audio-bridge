...
pub struct TrackAnalysisResponse {
    ...
    /// Derived heuristic values from the analysis pass.
...
pub struct TextMetadata {
    /// Text content (bio/notes).
    pub text: String,
    /// Optional source label/URL.
    #[serde(default)]
    pub source: Option<String>,
    /// Whether content is locked from automatic overwrite.
    #[serde(default)]
    pub locked: bool,
...
pub struct MediaAssetInfo {
    ...
    pub id: i64,
    /// Public URL for retrieving the asset.
    pub url: String,
...
pub struct ArtistProfileResponse {
    ...
    pub artist_id: i64,
    /// Language tag for text fields.
    pub lang: String,
    /// Optional artist biography.
    pub bio: Option<TextMetadata>,
    /// Optional artist image asset.
...
pub struct AlbumProfileResponse {
    ...
    /// Edition label (for example remaster name).
    #[serde(default)]
    pub edition_label: Option<String>,
    /// Optional album image asset.
    pub image: Option<MediaAssetInfo>,
}
...
pub struct ArtistProfileUpdateRequest {
    ...
    /// Lock flag for biography.
    #[serde(default)]
    pub bio_locked: Option<bool>,
    /// Source label/URL for biography.
    #[serde(default)]
    pub source: Option<String>,
}
...
pub struct LocalPlaybackRegisterRequest {
    ...
    pub name: String,
    /// Stable client id used to refresh registrations.
...
pub struct LocalPlaybackRegisterResponse {
    /// Registered local playback session id.
    pub session_id: String,
    /// Endpoint URL for resolving playable stream URLs.
    pub play_url: String,
}
...
pub struct LocalPlaybackPlayRequest {
    /// Track id to resolve for playback.
    pub track_id: i64,
}
...
pub struct LocalPlaybackSessionInfo {
    ...
    /// Time since last heartbeat/activity in milliseconds.
...
pub struct LocalPlaybackSessionsResponse {
    /// Registered local playback sessions.
...
pub struct SessionCreateRequest {
    ...
    /// Lease TTL in seconds (`0` = never expires).
    #[serde(default)]
...
pub struct SessionCreateResponse {
    /// Created or refreshed session id.
    pub session_id: String,
    /// Effective lease TTL in seconds.
...
pub struct SessionHeartbeatRequest {
    /// Client state marker (for example `foreground`/`background`).
    pub state: String,
    /// Optional battery value in range `0.0..=1.0`.
    #[serde(default)]
...
pub struct SessionLockInfo {
    /// Lock key (output id or bridge id).
    pub key: String,
    /// Owning session id.
    pub session_id: String,
}
...
pub struct SessionDetailResponse {
    ...
    /// Effective lease TTL in seconds.
    pub lease_ttl_sec: u64,
    /// Last heartbeat state marker.
    #[serde(default)]
    pub heartbeat_state: Option<String>,
    /// Last reported battery value.
    #[serde(default)]
    pub battery: Option<f32>,
}
...
pub struct SessionSelectOutputRequest {
    ...
    /// Force takeover when output/bridge is already owned.
    #[serde(default)]
    pub force: bool,
}
...
pub struct OutputInUseError {
    /// Error code (`output_in_use`).
    pub error: String,
    /// Requested output id.
    pub output_id: String,
    /// Session that currently holds the output lock.
...
pub struct SessionReleaseOutputResponse {
    /// Session id.
    pub session_id: String,
    /// Released output id, if one was bound.
    #[serde(default)]
